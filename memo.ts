...
// .like('column', '%CaseSensitive%') -> 컬럼값에 특정 값이 포함되어 있는지 검사(대소문자 구분)
// .ilike('column', '%CaseInsensitive%') -> 컬럼값에 특정 값이 포함되어 있는지 검사(대소문자 구분하지 않음)
// .is('column', null) -> 컬럼의 값이 NULL인지 검사
// .in('column', ['Array', 'Values']) -> 배열 안에 있는 값이 하나라도 컬럼 값과 매칭되는지 검사
